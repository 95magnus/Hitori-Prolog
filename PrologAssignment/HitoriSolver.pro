...
floodFill(C, [X,Y|Is], Chl):-
  ...
  ( X0 >= 0 -> elemAt(Chl, X0, Y, C0), (C0 = 0 -> flatten([X0, Y, Is], I0), floodFill(C, I0, Chl1) ; true) ; true),
  ( X1 <  S -> elemAt(Chl, X1, Y, C1), (C1 = 0 -> flatten([X1, Y, Is], I1), floodFill(C, I1, Chl1) ; true) ; true),
  ( Y0 >= 0 -> elemAt(Chl, X, Y0, C2), (C2 = 0 -> flatten([X, Y0, Is], I2), floodFill(C, I2, Chl1) ; true) ; true),
  ( Y1 <  S -> elemAt(Chl, X, Y1, C3), (C3 = 0 -> flatten([X, Y1, Is], I3), floodFill(C, I3, Chl1) ; true) ; true),
  floodFill(C, Is, Chl1), !.
